...
~MIRYOKU_ALPHAS=RSTHD~

Changes to RSTHD: ~E~ is moved to the right, and ~Enter~ takes the place of semicolon on Base.

Changes to Miryoku: ~Backspace~ is moved to the secondary thumb, and the primary thumb has ~E~ on Base and ~Enter~ on Nav.

Not available with ~MIRYOKU_LAYERS=FLIP~, ~MIRYOKU_NAV=VI~, or ~MIRYOKU_NAV=INVERTEDT~.
...
#+NAME: thumbs-rsthd
| U_NP  | U_NP  | ESC   | SPC   | TAB   | BSPC  | E     | DEL   | U_NP  | U_NP  |
...
#include "miryoku_alternatives.h"
      ...
      #define MIRYOKU_LAYER_BASE MIRYOKU_ALTERNATIVES_BASE_RSTHD
    #elif defined (MIRYOKU_ALPHAS_WORKMAN)
    ...
    #elif defined (MIRYOKU_ALPHAS_RSTHD)
      #define MIRYOKU_LAYER_NAV MIRYOKU_ALTERNATIVES_NAV_RSTHD
    #else
...
<<table-layer-taphold(alphas_table=rsthd, thumbs_table=thumbs-rsthd)>>
...
<<table-layer-half(half_table=nav-r-rsthd, mode="r")>>
